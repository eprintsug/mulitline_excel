...
sub input_file
{
	...
	foreach my $row ( $sheet->{MinRow}+1..$sheet->{MaxRow} )
	{
		...
		foreach my $col ( $sheet->{MinCol}..$max_col )
		{
	...
	foreach my $eprintid ( keys %{$by_eprint} )
	{
		...
		foreach my $row ( sort keys %{$rowsdata} )
		{
			...
			foreach my $dataid ( keys %{$rowdata} )
			{
				...
				# Map Microsoft smart quotes and friends, otherwise they'll turn into 0xfffd
				if( $rowdata->{$dataid} =~ /[\x82-\x9F]/ )
				{
					$rowdata->{$dataid} = Encode::decode( 'cp-1252', $rowdata->{$dataid} );
				}

				# need to make sure strings are properly flagged as utf-8 or else they'll be chopped off	
				if( !utf8::is_utf8( $rowdata->{$dataid} ) )
				{
					utf8::upgrade( $rowdata->{$dataid} )
				}
